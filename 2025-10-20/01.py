...
# Imprime na tela resultados da última query executada pelo cursor
...
def mostra_todos_dados(cursor):
    ...
    if resposta == 'S':
        with open('resultado_query.json', 'w', encoding='utf-8') as arquivo:
            json.dump(resultados_convertidos, arquivo, indent=4, ensure_ascii=False)

    #mostra_resultados_query(cursor)
    
# busca um nome na tabela de estudantes
def mostra_resultados_busca(cursor, nome_procurar):
    query = 'SELECT id, rm, nome, curso, turma FROM ESTUDANTES WHERE nome = :nome'
...
# busca um RM na tabela de estudantes
def mostra_resultados_busca_rm(cursor, rm):
    query = 'SELECT id, rm, nome, curso, turma FROM ESTUDANTES WHERE rm = :rm'
...
# insere novo estudante na tabela
